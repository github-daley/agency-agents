...
> NEXUS パイプライン内の任意のエージェントを起動するためのすぐに使えるプロンプトテンプレートです。`[PLACEHOLDERS]` をカスタマイズしてコピーし、デプロイしてください。
...
### Agents Orchestrator — Dev↔QA ループ管理
...
| シチュエーション | プライマリプロンプト | サポートプロンプト |
|-------------------|------------------------|---------------------|
| 新規プロジェクトの開始 | Orchestrator — Full Pipeline | — |
| 機能の構築 | Orchestrator — Dev↔QA Loop | Developer + Evidence Collector |
| バグ修正 | Backend/Frontend Developer | API Tester or Evidence Collector |
| キャンペーン実行 | Content Creator | Social Media Strategist + platform agents |
| ローンチ準備 | See Phase 5 Playbook | All marketing + DevOps agents |
| 月次報告 | Executive Summary Generator | Analytics Reporter + Finance Tracker |
| インシデント対応 | Infrastructure Maintainer | DevOps Automator + relevant developer |
| マーケットリサーチ | Trend Researcher | Analytics Reporter |
