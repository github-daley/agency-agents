...
> NEXUS パイプライン内のあらゆるエージェント間の引き渡しに使う標準テンプレートです。一貫した引き渡しはコンテキストの喪失を防ぎ、マルチエージェントの協調失敗の主要原因を解消します。

---

## 1. 標準引き渡しテンプレート

作業のエージェント間移行全般で使用します。
...
Evidence Collector や他の QA エージェントがタスクを承認した場合に使用します。
...
Evidence Collector や他の QA エージェントがタスクを却下した場合に使用します。
...
| シチュエーション | 使うテンプレート |
|------------------|------------------|
| 別のエージェントに作業を割り当てる | Standard Handoff (#1) |
| QA がタスクを承認した | QA PASS (#2) |
| QA がタスクを却下した | QA FAIL (#3) |
| タスクが 3 回以上失敗した | Escalation Report (#4) |
| フェーズ間を移動する | Phase Gate Handoff (#5) |
| スプリント終了時 | Sprint Handoff (#6) |
| システムインシデント | Incident Handoff (#7) |
